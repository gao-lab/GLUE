    ...
    def transfer_weights_from(self, other: "BatchedTransferrable") -> None:
        ...
        for key, target in chain(self.named_parameters(), self.named_buffers()):
            ...
            if key in self.batched_weights and len(self.batches):
                ...
                mask = index >= 0
                source = source[:, mask]
                index = torch.as_tensor(index[mask], device=target.device).expand_as(
                    source
                )
...
class DataDecoder(BatchedTransferrable):
    ...
    @property
    def batched_weights(self) -> List[str]:
        return [f"linear_{layer}.weight" for layer in range(self.h_depth)] + [
            "loc.weight"
        ]
...
class Discriminator(torch.nn.Sequential, BatchedTransferrable):
    def __init__(
        ...
        self.n_batches = len(
            self.batches
        )  # NOTE: Do not add 1, or we can't feed b properly
...
class SCCLUE(torch.nn.Module):
    def __init__(
        self,
        x2u: Mapping[str, CompositeDataEncoder],
...
class SCCLUETrainer(Trainer):
    def __init__(
        ...
        **kwargs,
        ...
        if self.net.u2c:
            ...
            self.vae_optim = getattr(torch.optim, optim)(
                chain(self.net.x2u.parameters(), self.net.u2x.parameters()),
                lr=self.lr,
                **kwargs,
            )
    ...
    def compute_losses(
        ...
        usamp_split = {
            k: torch.stack(
                torch.tensor_split(usamp[k], len(net.keys), dim=1)
        ...
        x_nll = {
            k: -net.u2x[k](
                usamp_split[k][i], xbch[k], None if l[k] is None else l[k][:, i]
            )
            .log_prob(x[k])
            .mean()
        ...
        usamp_mean_split = torch.stack(
            torch.tensor_split(usamp_mean, len(net.keys), dim=1)
        )
    ...
    def fit(  # pylint: disable=arguments-renamed
        ...
        data.getitem_size = max(
            1, round(batch_size / config.DATALOADER_FETCHES_PER_BATCH)
        )
        data_train, data_val = data.random_split(
            [1 - val_split, val_split], random_state=random_seed
        )
...
class SCCLUEModel(Model):
    ...
    def __init__(
        ...
        if shared_batches:
            ...
            du_batches = []
        du = Discriminator(
            latent_dim * len(self.modalities),
            len(self.modalities),
            batches=du_batches,
            h_depth=du_h_depth,
            h_dim=du_h_dim,
    ...
    def encode_data(
        ...
        data_loader = DataLoader(data, batch_size=1, shuffle=False, drop_last=False)
        result = []
        for x, xrep, *_ in data_loader:
            result.append(
                encoder(
                    x.to(self.net.device),
                    xrep.to(self.net.device),
    ...
    @torch.no_grad()
    def cross_predict(
        self, keys: Tuple[str, str], adata: AnnData, batch_size: int = 128
    ) -> AnnData:
        self.net.eval()
        ...
        data = AnnDataset(
            [adata], [self.modalities[source_key]], mode="eval", getitem_size=batch_size
        )
        data_loader = DataLoader(data, batch_size=1, shuffle=False, drop_last=False)
        ...
        for x, xrep, *_ in data_loader:
            u, _ = x2u(
                x.to(self.net.device), xrep.to(self.net.device), lazy_normalizer=True
            )
            umean = F.normalize(u.mean, dim=1) if self.normalize_u else u.mean
            umean_split = torch.stack(
                torch.tensor_split(umean, len(self.net.keys), dim=1), dim=0
            )
            result.append(
                u2x(
