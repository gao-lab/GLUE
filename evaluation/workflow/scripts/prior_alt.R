...
distance_abbr <- function(x) {
    if (x < 1000) {
        return(as.character(x))
    } else (
        return(paste0(as.character(round(x / 1000)), "k"))
    )
}

df_summarise <- df %>%
    mutate(prior_type = paste(
        tools::toTitleCase(gene_region),
        Map(distance_abbr, extend_range),
        sep = "±"
    )) %>%
    group_by(dataset, method, prior_type) %>%
    summarise(
...
gp <- ggplot(data = df_summarise %>% filter(dataset %in% paired_datasets), mapping = aes(
    ...
    scale_x_discrete(name = "Prior knowledge") +
...
gp <- ggplot(data = df_summarise, mapping = aes(
    x = prior_type, y = mean_average_precision,
...
gp <- ggplot(data = df_summarise, mapping = aes(
    x = prior_type, y = seurat_alignment_score,
...
gp <- ggplot(data = df_summarise, mapping = aes(
    x = prior_type, y = avg_silhouette_width,
...
gp <- ggplot(data = df_summarise, mapping = aes(
    x = prior_type, y = avg_silhouette_width_batch,
...
gp <- ggplot(data = df_summarise, mapping = aes(
    x = prior_type, y = neighbor_conservation,
...
gp <- ggplot(data = df_summarise, mapping = aes(
    x = prior_type, y = graph_connectivity,
