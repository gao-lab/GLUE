...
def test_window_graph(bed_file, graph):
    ...
    result = scglue.genomics.window_graph(
        ...
        attr_fn=lambda l, r, d: {
            "dist": abs(d),
            "weight": 1 / abs(d) if d != 0 else 1.0,
        },
    )
    cmp_graphs(result, graph)


def test_anchored_guidance_graph(rna, atac, gtf_file):  # NOTE: Smoke test
    ...
    scglue.genomics.anchored_guidance_graph(
        ...
        propagate_highly_variable=True,
    )
...
def test_regulatory_inference(rna):
    ...
    with pytest.raises(ValueError):
        _ = scglue.genomics.regulatory_inference(
            rna.var_names, rna.X.T, skeleton, alternative="xxx", random_state=0
        )
    with pytest.raises(ValueError):
        _ = scglue.genomics.regulatory_inference(
            rna.var_names, rna.X, skeleton, alternative="two.sided", random_state=0
        )
    with pytest.raises(ValueError):
        _ = scglue.genomics.regulatory_inference(
            rna.var_names,
    ...
    _ = scglue.genomics.regulatory_inference(
        rna.var_names, rna.X.T, skeleton, alternative="two.sided", random_state=0
    )  # NOTE: Smoke test
    _ = scglue.genomics.regulatory_inference(
        rna.var_names,
        [rna.X.T, rna.X.T],
    ...
    _ = scglue.genomics.regulatory_inference(
        rna.var_names, [rna.X.T], skeleton, alternative="less", random_state=2
    )  # NOTE: Smoke test
...
def test_write_links(rna_pp, tmp_path):
    ...
    reginf = scglue.genomics.regulatory_inference(
        rna_pp.var_names, rna_pp.X.T, skeleton, alternative="two.sided", random_state=0
    )
...
def test_cis_regulatory_ranking(rna, atac, guidance):
    _ = scglue.genomics.cis_regulatory_ranking(
        guidance, guidance.reverse(), rna.var_names, atac.var_names, rna.var_names
    )  # NOTE: Smoke test
